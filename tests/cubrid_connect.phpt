...
// invalid db
$conn2 = cubrid_connect('localhost', '33000', 'invalid_db', 'dba', '');
if($conn2 == false){
     printf("[007] [%d] %s\n", cubrid_error_code(), cubrid_error_msg());
}
 
// invalid password
$conn2 = cubrid_connect('localhost', '33000', 'demodb', 'dba', '222');
if($conn2 == false){
     printf("[008] [%d] %s\n", cubrid_error_code(), cubrid_error_msg());
}
...
$conn1 = cubrid_connect('localhost', '33000', 'demodb', 'invalid_user', '');
if($conn1 == false){
     printf("[010] [%d] %s\n", cubrid_error_code(), cubrid_error_msg());
}
