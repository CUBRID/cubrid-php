...
$req1 = cubrid_execute($conn,"select c1,c2,c3,c4,c5,c6,c7,c8,c9,CLOB_TO_CHAR(c10),BLOB_TO_BIT(c11) from fetch_tb;");
if (!$req1) {
...
$req2=cubrid_execute($conn,"select c1,c2,c3,c4,c5 from fetch_tb where c3=2");
if (!$req2) {
    printf("req2 [%d] %s\n", cubrid_error_code(), cubrid_error_msg());
}else{
...
$req6=cubrid_execute($conn,"select c1,c2,c3,c4,c5,c6,c7,c8,c9,CLOB_TO_CHAR(c10),BLOB_TO_BIT(c11) from fetch_tb;");
...
Warning: cubrid_fetch() expects parameter 2 to be integer, string given in %s on line %d
[002] Expected NULL, [0] 
